...
var ReplSetTest = function(opts) {
    ...
    function _constructFromExistingSeedNode(seedNode) {
        ...
        self.nodes = existingNodes.map(node => new MongoBridge({hostName: node.split(':')[0], port: node.split(':')[1], dest: node.split(':')[0] + ":" + eval(parseInt(node.split(':')[1], 10) + 10), connectExistingBridge: true}));
